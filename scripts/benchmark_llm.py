...
TEST_CASES = [
    {
        "name": "Single Tool",
        "prompt": "Turn off the living room light"
    },
    {
        "name": "Multi Tool",
        "prompt": "Check the temperature in the bedroom and save it to the log"
    },
    {
...
def check_accuracy(test_name: str, response: str) -> str:
    ...
    if test_name == "Single Tool":
        ...
        if "book_flight" in response_lower or "bookflight" in response_lower:
            return "Fail"
        if "[]" in response_lower or "cannot" in response_lower or "don't have" in response_lower or "unable" in response_lower:
            return "Pass"
