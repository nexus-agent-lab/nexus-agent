...
def list_entities():
    ...
    entities = []
    for i in range(800):
        entities.append({"entity_id": f"sensor.temp_{i}", "state": "20.5", "attributes": {"friendly_name": f"Temp Sensor {i}"}})
    entities.append({"entity_id": "light.living_room", "state": "on", "attributes": {"friendly_name": "Living Room Light"}})
...
### CRITICAL RULES
1. **DATA OVERFLOW**: If a tool returns "Data offloaded to file...", you MUST use `python_sandbox` to read that file and filter it.
2. **NO CHATTY CODE**: 
...
async def run_test():
    ...
    messages = [
        SystemMessage(content=EXPERIMENTAL_SYSTEM_PROMPT),
