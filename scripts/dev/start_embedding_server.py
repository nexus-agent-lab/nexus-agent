...
async def create_embeddings(request: EmbeddingRequest):
    ...
    if isinstance(inputs, str):
        ...
        print("Warning: Received tokenized input (integers). This server expects strings.")
        raise HTTPException(
            status_code=422, detail="This local embedding server requires raw text strings, not tokenized integers."
        )
    ...
    try:
        ...
        data = [EmbeddingObject(index=i, embedding=emb) for i, emb in enumerate(embeddings)]

        return EmbeddingResponse(data=data, model=request.model or "local-model")
