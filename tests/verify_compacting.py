...
from app.models.session import SessionMessage, SessionSummary
...
async def verify_auto_compacting():
    ...
    class MockLLM:
        async def ainvoke(self, prompt):
            return AIMessage(
                content="[Summary] The user and assistant exchanged 15 messages testing the context window compaction feature."
            )
    ...
    async with TestSessionLocal() as db:
        # Check Summaries
        summaries = (
            (await db.execute(select(SessionSummary).where(SessionSummary.session_id == session.id))).scalars().all()
        )
        ...
        archived_count = (
            await db.execute(
                select(func.count()).where(SessionMessage.session_id == session.id, SessionMessage.is_archived)
            )
        ).scalar()
